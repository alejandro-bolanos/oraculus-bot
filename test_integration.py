...
def integration_setup():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        master_data = pd.DataFrame({
            ...
            "dataset": ["public" if i <= 30 else "private" for i in range(1, 101)]  # 30% público
        })
        ...
        config = {
            ...
            "gain_thresholds": [
                {"min_score": 1000, "category": "excellent", "message": "¡Modelo excepcional!", "emoji": "🏆"},
                {"min_score": 500, "category": "good", "message": "Buen modelo", "emoji": "👍"},
                {"min_score": 0, "category": "basic", "message": "Modelo básico", "emoji": "💪"},
                {"min_score": -1000, "category": "poor", "message": "Necesita mejoras", "emoji": "📚"},
            ],
            "badges": {
...
class TestMultiUserScenarios:
    ...
    @patch("oraculus_bot.zulip.Client")
    @patch("oraculus_bot.requests.get")
    def test_competition_with_multiple_students(self, mock_requests, mock_zulip_client, integration_setup):
...
class TestErrorHandlingAndEdgeCases:
    ...
    def test_database_corruption_recovery(self, mock_zulip_client, integration_setup):
        ...
        with pytest.raises(Exception):
            bot.save_submission(
                user_info, "test_model", "/path", "checksum",
                public_results, private_results, 2, "good"
            )
...
class TestDataIntegrity:
    ...
    @patch("oraculus_bot.zulip.Client")
    @patch("oraculus_bot.requests.get")
    def test_duplicate_detection_accuracy(self, mock_requests, mock_zulip_client, integration_setup):
...
class TestRobustnessAndRecovery:
    ...
    def test_bot_restart_data_persistence(self, mock_zulip_client, integration_setup):
        ...
        submission_id = bot1.save_submission(
            user_info, "persistent_model", "/path", "checksum123",
